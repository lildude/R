...
app_name <- Sys.getenv("STRAVA_NAME") # name of app on Strava
app_client_id  <- Sys.getenv("STRAVA_CLIENT_ID") # an integer, assigned by Strava
myid <- Sys.getenv("STRAVA_ID") # another integer you can find on your Strava profile
app_secret <- Sys.getenv("STRAVA_APP_SECRET") # an alphanumeric secret, assigned by Strava
goog_key <- Sys.getenv("GOOGLE_MAPS_API_KEY") # from your own Google API account
